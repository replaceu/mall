...
<script>
import { WareInfoSaveApi, WareInfoUpdateApi,WareInfoApi } from "@/api/ware/wareinfo.js"
  ...
  methods: {
    init(id) {
      ...
      this.$nextTick(() => {
        ...
        if (this.dataForm.id) {
          WareInfoApi(this.dataForm.id).then(data => {
    ...
    dataFormSubmit() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          if (this.dataForm.id) {
            WareInfoUpdateApi(data).then(() => {
              SuccessMessage("修改成功");
              this.visible = false
              this.$emit('refreshDataList')
            })
          } else {
            WareInfoSaveApi(data).then(() => {
              SuccessMessage("添加成功");
              this.visible = false
              this.$emit('refreshDataList')
            })
