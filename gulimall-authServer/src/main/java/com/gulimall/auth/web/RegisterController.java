...
public class RegisterController {
	...
	public CommonResult<String> sendCode(@RequestParam("phone") String phone) {
		if (StringUtils.isEmpty(phone)) { return CommonResult.fail("手机号不能为空"); }
		if (!phone.matches("^((13[0-9])|(14[5|7])|(15([0-3]|[5-9]))|(18[0,5-9]))\\d{8}$")) { return CommonResult.fail("手机号格式错误"); }
		// 1、接口防刷
	...
	public String register(@Valid UserRegisterVo userRegisterVo, String originUrl, BindingResult result, RedirectAttributes redirectAttributes, HttpSession session) {
		...
		if (result.hasErrors()) {
			Map<String, String> errorMap = result.getFieldErrors().stream().collect(Collectors.toMap(e -> e.getField(), e -> e.getDefaultMessage()));
			//            请求转发 的区别
