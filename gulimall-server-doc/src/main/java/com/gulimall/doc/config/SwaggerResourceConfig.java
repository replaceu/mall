	...
	public List<SwaggerResource> get() {
		...
		gatewayProperties.getRoutes().stream().filter(routeDefinition -> routes.contains(routeDefinition.getId())).forEach(route -> {
			route.getPredicates().stream()
					// 拿到所有的 path
					.filter(predicateDefinition -> ("Path").equalsIgnoreCase(predicateDefinition.getName())).filter(predicateDefinition -> !predicateDefinition.getArgs().get(NameUtils.GENERATED_NAME_PREFIX + "0").startsWith("/api")).forEach(predicateDefinition -> {
