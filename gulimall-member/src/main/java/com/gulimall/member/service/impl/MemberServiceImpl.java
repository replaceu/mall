...
public class MemberServiceImpl extends ServiceImpl<MemberDao, MemberEntity> implements MemberService {
	...
	public void checkPhoneUnique(String phone) throws BusinessException {
		...
		if (count > 0) { throw new BusinessException(MemberErrorCode.PHONE_EXIST_EXCEPTION); }
	}
	...
	public void checkUsernameUnique(String username) throws BusinessException {
		...
		if (count > 0) { throw new BusinessException(MemberErrorCode.USERNAME_EXIST_EXCEPTION); }
	}
	...
	public MemberEntity login(String loginAccount, String password) {
		...
		if (memberEntity != null && passwordEncoder.matches(password, memberEntity.getPassword())) { return memberEntity; }
		//登陆失败
		return null;
	...
	public MemberEntity login(SocialUser socialUser) {
		...
		try {
			...
			ResponseEntity<WeiboSocialUserInfo> response = new RestTemplate().getForEntity(url, WeiboSocialUserInfo.class);
